...
# the jobdir, captured BEFORE any user code: a block's setwd() (an
# ordinary idiom) keeps its effect across blocks — session state — but
# can no longer orphan the driver's protocol files (field incident)
jobdir <- getwd()
...
repeat {
  ...
  rc_f <- jp(sprintf("blocks/%04d.rc", n))
  code_f <- jp(sprintf("blocks/%04d.code", n))
  if (file.exists(rc_f)) { n <- n + 1L; next }
  if (!file.exists(code_f)) { Sys.sleep(0.2); next }
  writeLines(as.character(n), jp("current_block"))
  art <- jp(sprintf("blocks/%04d.artifacts", n))
