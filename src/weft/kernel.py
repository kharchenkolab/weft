    ...
    def start(self, site: str, lang: str = "python",
        ...
        if env_id:
            ...
            if s_mode == "clone":
                ...
                # realization carries the interpreter. The FORWARD HOOK
                # keeps the live-install contract: the python driver puts
                # the session's (possibly future) layer at sys.path[0]
                # and invalidates import caches per block, so a later
